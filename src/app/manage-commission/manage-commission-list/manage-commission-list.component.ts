...
@Component({
  selector: 'pb-manage-commission-list',
  templateUrl: './manage-commission-list.component.html',
  styleUrls: ['./manage-commission-list.component.scss']
})
export class ManageCommissionListComponent implements OnInit {
  ...
  public displayedColumns = ['id', 'commission_type', 'fees', 'status', 'created_at', 'updated_at'];
  ...
  public getUsers() {
    ...
    // Inject service
    const manageCommissionListService = this.injector.get(ManageCommissionListService);
    const progress = this.injector.get(NgProgress);
    // Call API
    progress.start();
    this.isLoading = true;
    manageCommissionListService.getUsers(this.tableConfig.limit, this.tableConfig.pageNo,
      this.tableConfig.searchString, this.startDate, this.endDate, this.sortType,
      ...
      (response: HttpResponse<any>) => {
        if (response['code'] === 200) {
          ...
          // Append Table Data
          this.tableConfig.totalRecords = response['data']['commissionlogs']['totalRecords'];
          this.TABLE_DATA = response['data']['commissionlogs']['result'];
