...
@Component({
  selector: 'pb-press-category-list',
  templateUrl: './press-category-list.component.html',
  styleUrls: ['./press-category-list.component.scss']
})

export class PressCategoryListComponent implements OnInit, OnDestroy {
  ...
  public getBlogsList() {
      // Inject service
      const blogCategoryListService = this.injector.get(PressCategoryListService);
      ...
      blogCategoryListService.getBlogsList(this.tableConfig.limit, this.tableConfig.pageNo,
         this.tableConfig.searchString, this.filterTitle,
        this.selectedStatus, this.sortType, this.sortBy).subscribe(
